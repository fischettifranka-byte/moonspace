...
app.get('/api/fortune/today', (req, res) => {
  const fortunes = [
    ...
    '宜午后小憩','宜整理房间','宜给花浇水','宜发一条动态','宜听播客',
    '宜做拉伸运动','宜写日记','宜泡个热水澡','宜看一部老电影','宜学做新菜',
    '宜读一首诗','宜给老朋友发消息','宜整理相册','宜尝试新发型','宜种一盆绿植',
    '宜练习深呼吸','宜抄经静心','宜仰望星空','宜赏一幅画','宜折纸',
    '宜喂流浪猫','宜骑车漫游','宜逛菜市场','宜手写一封信','宜收集落叶',
    '宜夜读','宜焚一炉香','宜弹一曲琵琶','宜画一幅山水','宜饮一杯清酒',
    '宜编一个中国结','宜刻一方印章','宜折一枝梅花','宜放一盏河灯','宜荡一次秋千',
    '宜围炉夜话','宜踏青采薇','宜临水照花','宜听蝉鸣夏','宜扫雪烹茶'
  ];
  const avoids = [
    ...
    '忌空腹喝咖啡','忌跷二郎腿','忌憋着不去厕所','忌睡前刷短视频',
    '忌边充电边玩手机','忌不吃早餐','忌蒙头睡觉','忌背后说人',
    '忌口是心非','忌杞人忧天','忌得意忘形','忌妄下判断',
    '忌一曝十寒','忌临渊羡鱼','忌舍本逐末','忌随波逐流',
    '忌以貌取人','忌半途而废','忌对牛弹琴','忌缘木求鱼',
    '忌多言招祸','忌心浮气躁','忌患得患失','忌好为人师',
    '忌怒发冲冠','忌愁云满面','忌纸上谈兵','忌画地为牢'
  ];
  const blessings = [
    '今天会有好事发生 ✨','你是被月亮眷顾的人 🌙',
    ...
    '今天也要活得热气腾腾 🔥','但行好事，莫问前程 🛤️',
    '万里归来颜愈少，微笑，笑时犹带岭梅香 🌸','回首向来萧瑟处，归去，也无风雨也无晴 ☀️',
    '愿你所遇皆温柔，所行皆坦途 ✈️','人生如逆旅，我亦是行人 🚶',
    '竹杖芒鞋轻胜马，谁怕 🎋','会当凌绝顶，一览众山小 ⛰️',
    '海上生明月，天涯共此时 🌊','众里寻他千百度，蓦然回首，那人却在灯火阑珊处 🏮',
    '大鹏一日同风起，扶摇直上九万里 🦅','天生我材必有用，千金散尽还复来 💰',
    '仰天大笑出门去，我辈岂是蓬蒿人 😎','两岸猿声啼不住，轻舟已过万重山 ⛰️',
    '醉卧沙场君莫笑，古来征战几人回 🍷','晴空一鹤排云上，便引诗情到碧霄 🕊️',
    '落霞与孤鹜齐飞，秋水共长天一色 🌅','春江潮水连海平，海上明月共潮生 🌕',
    '人生得意须尽欢，莫使金樽空对月 🍶','山重水复疑无路，柳暗花明又一村 🌸',
    '沉舟侧畔千帆过，病树前头万木春 🌱','衣带渐宽终不悔，为伊消得人憔悴 💓',
    '昨夜西风凋碧树，独上高楼，望尽天涯路 🏯','春风又绿江南岸，明月何时照我还 🌿',
    '愿你被世界温柔以待 💫','今天的小确幸在路上啦 🍀',
    '宇宙山河浪漫，生活点滴温暖 🌍','万物皆有裂痕，那是光照进来的地方 ☀️',
    '你特别好，特别值得 💎','慢慢来，比较快 🐌'
  ];
  const lucky = ['金色','蓝色','绿色','粉色','紫色','橙色','白色','银色','鹅黄色','薄荷绿','珊瑚粉','天空蓝','黛青色','月白色','烟雨灰','琥珀色','松石绿','藕荷色','霁红色','杏黄色','靛蓝色','竹青色','酡红色','雪青色'];
  const luckyNumbers = ['3','6','7','8','9','11','12','16','21','25','28','33','36','42','48','52','56','63','72','81','88','99'];
  // seed = 日期 + 用户hash，同一天不同人看到不同的运势，同一人同一天保持一致
  const dateNum = parseInt(new Date().toISOString().slice(0, 10).replace(/-/g,''),10);
  const userHash = (req.session?.user || 'moon').split('').reduce((a,c)=>a+c.charCodeAt(0),0);
  const seed = dateNum + userHash;
  const rng = (max) => Math.abs(Math.floor(Math.sin(seed * 9301 + 49297) * 233280)) % max;
  res.json({
    ok: true,
    fortune: {
      fortune: fortunes[rng(fortunes.length)],
      avoid: avoids[rng(avoids.length + 7) % avoids.length],
      blessing: blessings[rng(blessings.length + 3) % blessings.length],
      luckyNumber: luckyNumbers[rng(luckyNumbers.length)],
      luckyColor: lucky[rng(lucky.length + 5) % lucky.length],
      rating: rng(5) + 1
    }
  });
});
